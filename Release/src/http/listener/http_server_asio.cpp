...
void hostport_listener::start()
{
    ...
    // #446: boost resolver does not recognize "+" as a host wildchar, use ANY
    tcp::endpoint endpoint;
    if ("+" == m_host) 
    {
        auto results = resolver.resolve("", m_port);
    ...
    m_acceptor->listen(0 != m_backlog ? m_backlog : SOMAXCONN);
...
will_deref_t asio_server_connection::handle_body(const boost::system::error_code& ec)
{
    ...
    else if (m_read < m_read_size) // there is more to read
    {
        auto writebuf = requestImpl->outstream().streambuf();
        writebuf
            .putn_nocopy(static_cast<const uint8_t*>(m_request_buf.data().data()), (std::min)(m_request_buf.size(), m_read_size - m_read))
